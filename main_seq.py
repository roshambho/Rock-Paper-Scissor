...
questions = [
  ...
  "Guess the # of people in the Morning Sadhana icon in the Sadhanapada app",
  ...
  "What's the best time to wake-up?",
   ...
   "On which date did Janmashthami happen this year ?",
  ...
  "What is the third wake-up chant in Sadhanapada stay area?",
  
     "On which date did Guru Pournima fall this year?",
...
options = {
   ...
   "On which date did Janmashthami happen this year ?":{'A': '8 Sep ','B': '5 Sep ','C': '6 Sep'},
...
"How many Ekadashis have occured in Sadhanapada 2023 ?": {'A': 'Nine','B': 'Ten ','C': 'Eight'},
 
  "What is the third wake-up chant in Sadhanapada stay area?":{'A': 'Guru Paduka Stotra','B': 'Aum namah Shivaya','C': 'Nirvana Shatakam'},

  "What's the best time to wake-up?":{'A':'3.40 am', 'B': 'Let the body wake up naturally', 'C': '4.30 am'},

"How many video calls does the applications team make per day ?": {'A': '53','B': '64','C': '73'},
 
"How many Sadhanapada batches have culminated till now ?": {'A': 'Five','B': 'Four','C': 'Six'},
...
"Who amongst the following celebrities have attended Sadhanapada ?": {'A':'Zayn Malik','B':'Machel Montano','C':'Jackson Browne'},
...
"What's the first wake-up chant in the Sadhanapada stay areas ?": {'A': 'Guru Paduka Stotra','B': 'Aum namah shivaya','C': 'Nirvana Shatakam'},
 
"What's the second wake-up chant in the Sadhanapada stay areas ?": {'A': 'Uyirnokkam','B': 'Nirvana Shatakam','C': 'Aum namah shivaya'},
 
 "What/who comes after the last wake-up music in the morning ?": {'A': 'Drum-beat Anna','B': 'Yogasana','C': 'Guru Pooja'},
...
"Guess the distance covered in 5 Ekadashi Pradakshinas ?": {'A': '14 km','B': '16 km','C': '15 km'},
 
 "Guess the # of people in the Morning Sadhana icon on the Sadhanapada app": {'A': 'Two','B': 'Three','C': 'One'},
 ...
 "What is the age criteria to be eligible for Sadhanapada ?": {'A': '21','B': '18','C': '19'},
 ...
 "How many times has mega cleaning happened till now ?": {'A': 'Five','B': 'Three','C': 'Four'},
 
 "On which date did Guru Pournima fall this year?":{'A': '4 July','B': '3 July','C': '5 July'}

}

correct_option_mapping = {

   "On which date did Janmashthami happen this year ?":'B',

    "What's the best time to wake-up?":'B',

  "Where is Isha yoga center located?":'C',
  
   "On which date did Guru Pournima fall this year?":'B',
  ...
  "How many Ekadashis have occured in Sadhanapada 2023 ?": 'C',
    ...
    "What is the third wake-up chant in Sadhanapada stay area?":'B',
  ...
  "Who amongst the following celebrities have attended Sadhanapada ?": 'B',
...
def display_options(frame, question):
    ...
    cv2.putText(frame, f"{optionA}", (600, 1810), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 3)
    cv2.putText(frame, f"{optionB}", (600, 2050), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 3)
    cv2.putText(frame, f"{optionC}", (2400, 1810), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255),3)
...
def show_ellipse(image, gesture):
    if gesture=='SCISSOR':
        center_coordinates = (950, 1770)
        
    elif gesture=='ROCK':
        center_coordinates = (2900, 1770)
      
    elif gesture=='PAPER':
        center_coordinates = (950, 2000)
     
    axesLength = (900, 130)
    angle = 0
    startAngle = 0
    endAngle = 360
    image = cv2.ellipse(image, center_coordinates, axesLength, angle, startAngle, endAngle, (31, 131, 245), 5)
    return image
...
def gesture_trial(cap):
    ...
    for sample_symbol in symbols:
        ...
        gesture_pic = cv2.resize(cv2.imread(sample_symbol),(1528, 860), interpolation=cv2.INTER_LINEAR)
        if (count_round==0):
            timer_gesture_trial = int(4)
        else:
        ...
        while timer_gesture_trial>0:
            ...
            image = cv2.putText(image, f"Choose the correct option by making a hand gesture (Rock/Paper/Scissor)", (600, 200), cv2.FONT_HERSHEY_COMPLEX, 2, (31, 131, 245), 3)
            image = cv2.putText(image, f"HOLD your gesture until the countdown timer hits 0!", (1000, 280), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 0, 255), 3)
            #image[ 0:300,995:2845] = practice_time_image #enlarged banner
            image [ 0:154, 1142:2700] = practice_time_image
            ...
            display_question_and_options(image, -1, 0)
            image = show_ellipse(image, mode_of_prediction)
            
            
            
            image[450:1310,0:1528] = gesture_pic
            #print("here")
            ...
            if cur - prev >= 1:
                ...
                image = cv2.putText(image, "00:", (3440, 420), font, 4, (0, 0, 255), 3, cv2.LINE_AA)
                image = cv2.putText(image, f"{str(trial_timer)}", (3640, 420), font, 4, (0, 0, 255), 3, cv2.LINE_AA)
                cv2.imshow('Trial',image)
                cv2.waitKey(25)
...
demo_image = cv2.resize(cv2.imread('images/demo_image.jpeg'), (3840,2160), interpolation = cv2.INTER_CUBIC)
...
while True:
    ...
    if( global_qn_count/4 == 6):
        question_sequence = range(24,28)
        global_qn_count = 0
    global_qn_count+=4
    ...
    qn_count = 0
    print("total qns",len(questions))
    print("total mapping",len(correct_option_mapping))
    ...
    while 48 < r <= 52:  # Ascii values for 0 & 9
        ...
        ########## REAL-TIME RECOGNITION START ##########
            ...
            show_ellipse(image, mode_of_prediction)
